...
class pwspecialspage extends Module {
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Страница акций');
        $this->description = $this->l('Модуль с проводимыми акциями');
    ...
    {
        $meta = new Meta();
        $meta->page = 'module-pwspecialspage-akcii';
        $meta->title = 'Акции в интернет магазине';
        $meta->url_rewrite = 'akcii';
        $meta->add();
        if (!parent::install() ||
...
//        }
    ...
    public function uninstall() {
        if (!parent::uninstall() ||
            !Configuration::deleteByName('pwspecialspage')
        ) {
            return false;
        }

        Db::getInstance()->delete(_DB_PREFIX_ . 'meta', 'page =' . '"module-pwspecialspage-akcii"');
        return true;
    }
    ...
    private function _displayForm() {
        ...
        if (file_exists(dirname(__FILE__) . '/data.xml'))
            if (!$xml = @simplexml_load_file(dirname(__FILE__) . '/data.xml'))
    ...
    public function hookHeader($params) {
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            ...
            Tools::addCSS($this->_path . 'pwspecialspage.css', 'all');
        } else {
            $this->context->controller->addJqueryPlugin('fancybox');
            $this->context->controller->addCSS($this->_path . 'pwspecialspage.css', 'all');
        }
    }
